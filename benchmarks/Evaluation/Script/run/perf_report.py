...
def plot_per_model(
    ...
    fig, ax = plt.subplots(figsize=(max(6, 1.1 * len(apps) + 2), 5))
...
def plot_combined(
    ...
    legend_cols = max(1, legend_entries)
    legend_rows = 1
    fig_width = max(10.5, 2.35 * n_groups + 0.85 * n_bars)
    fig_height = 6.2 + 0.5 * max(0, legend_rows - 1)
    ...
    fig.tight_layout(rect=(0.0, 0.0, 1.0, 0.93 + 0.03 * max(0, legend_rows - 1)))
    fig.savefig(out_path, dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def _ordered_apps(stats: list[MetricStat], baseline_app: str) -> list[str]:
    """Stable order: baseline first, then other apps alphabetically."""
    apps = sorted({s.app for s in stats}, key=str.lower)
...
def run(cfg: Config) -> Path:
    ...
    apps_order = _ordered_apps(stats, baseline)
