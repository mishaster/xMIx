...
control which modes appear in which charts. A mode can appear in multiple groups.

Usage:
    # Default: all modes in one group
    python excel_to_charts.py All_results_llama.xlsx \
...
Config file format (groups.json):
    {
        ...
        "groups": [
            ...
            {
                "name": "Steering Variants + Baseline",
                "match": ["vanilla", "steer"]
            }
        ]
    }
...
def filter_records_by_group(records, match_patterns):
    """Filter records to those matching any of the given substring patterns."""
    ...
    for rec in records:
        ...
        for pattern in match_patterns:
            if pattern.lower() in mode_lower and rec["mode"] not in seen:
                result.append(rec)
                seen.add(rec["mode"])
                break
    return result
...
"""
    ...
    for model_data in all_data:
        ...
        html += f'  <h2 class="model-title">Model: {model}</h2>\n'

        for group in groups:
            group_name = group["name"]
            group_records = filter_records_by_group(records, group["match"])
                ...
                for idx, rec in enumerate(group_records):
                    ...
                    if baseline_val is not None:
                        ...
                        if is_baseline:
                            ...
                            # For throughput, higher is good; for latency, lower is good
                            if higher_is_better:
...
Config file example (groups.json):
  {
    ...
    "groups": [
      ...
      { "name": "All Modes",                   "match": ["*"] },
      { "name": "Steering Variants + Baseline", "match": ["vanilla", "steer"] }
    ]
  }

"baseline": substring to identify the reference mode (e.g. "vanilla").
  Each bar shows % diff vs the baseline. Omit to disable.
Each group's "match" list uses substring matching (case-insensitive).
Use ["*"] to include all modes. A mode can appear in multiple groups.
        """)
