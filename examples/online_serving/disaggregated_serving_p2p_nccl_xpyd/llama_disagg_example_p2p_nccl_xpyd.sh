...
main() {
    ...
    # Run Benchmark
    # =============================================================================
    OUTPUT_FILE="all_runs_results_$(date +"%Y%m%d_%H%M").jsonl"
    cd /home/michael.blum/vLLM_Patched/
    > $OUTPUT_FILE
    for i in {1..15}
    do
            vllm bench serve --port 10001 \
                ...
                --num-prompts 1000 --burstiness 1 | tee temp_run.json
        cat temp_run.json >> $OUTPUT_FILE 
        echo "" >> $OUTPUT_FILE
    done
